...
/// <summary>CVL compare page. Wires bucket-bar clicks to a negative-set filter on the inner grid.</summary>
public partial class CvlCompareView : UserControl
{
    public CvlCompareView()
    {
        AvaloniaXamlLoader.Load(this);
        DataContextChanged += OnDataContextChanged;
    }

    private CvlCompareViewModel? _vmHooked;

    private void OnDataContextChanged(object? sender, EventArgs e)
    {
        if (_vmHooked is not null) _vmHooked.Buckets.Changed -= OnBucketsChanged;
        _vmHooked = DataContext as CvlCompareViewModel;
        if (_vmHooked is not null) _vmHooked.Buckets.Changed += OnBucketsChanged;
    }
